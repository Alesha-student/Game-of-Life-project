...
#blocks = [[randint(0, 1) for l in range(width_count)] for j in range(height_count)]#Random pattern

blocks = [[1 if i == width_count // 2 or  (j == height_count // 2) else 0 for i in range(width_count)] for j in range(height_count)]#Cross pattern
...
for i in range(height_count):
    ...
    blocks[height_count-i-1][i+(width_count-height_count) // 2]=1'''#Diogonal pattern
